...
class TradingController extends Controller
{
    ...
    public function sendAlert($symbole,$stopLoss,$id_stop_loss,$prix_trading)
    {
        ...
        foreach ($stopLossSymboles as $stopLossSymbole){
            ...
            $response = $client->request('GET', 'https://www.alphavantage.co/query', [
                'query' => [
                    ...
                    'interval' => '1min',
                    'apikey' =>  $stopLossSymbole->symbole_boursier,
                ]
            ]);
            $data = json_decode($response->getBody(), true);
            $currentPrice = $data['Time Series (1min)'][array_key_first($data['Time Series (1min)'])]['4. close'];
            $datePresentation = $data['Meta Data']["3. Last Refreshed"];
            ...
            DB::table("actifs_financiers_montant")
                ->insert(array(
                    ...
                    'date_creation' => $datePresentation,
                ));
